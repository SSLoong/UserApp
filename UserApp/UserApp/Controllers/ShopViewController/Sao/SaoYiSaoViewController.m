...
static NSString *saoText = @"将二维码放入框内，即可自动扫描";
...
NSLog(@"%@",resultString);
    NSString *subString = @"{\"sub\":\"singleorder\",\"data\":{\"id\":\"";
    
    if ([resultString rangeOfString:subString].location != NSNotFound ) {
        
        NSString *str = [resultString substringFromIndex:35];
        
        [self cartQrcode:[str substringToIndex:str.length-3]];
    ...
    NSString *storeString = @"/store?shop_id=";
    
    if ([resultString rangeOfString:storeString].location != NSNotFound) {
        
        NSString *str = [resultString substringFromIndex:resultString.length-18];
        NSLog(@"%@",str);
        ScanStoreViewController *vc = [[ScanStoreViewController alloc]init];
    ...
    NSString *orderString = @"/rqcode/buy/";
    
    if ([resultString rangeOfString:orderString].location != NSNotFound) {
        
     
        NSRange range =  [resultString rangeOfString:@"/" options:NSBackwardsSearch];
        WYBLog(@"%@",NSStringFromRange(range));
        NSString * newStr = [resultString substringFromIndex:range.location +1 ];
        WYBLog(@"%@",newStr);
        
        [self cartQrcode:newStr];
        return;
    }

    

    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"扫描结果" message:@"无效二维码" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
    [alert show];

}


-(void)cartQrcode:(NSString *)cart_id{

    _hud = [AppUtil createHUD];
    _hud.labelText = @"请稍后...";
    _hud.userInteractionEnabled = YES;
    
    [AFHttpTool cartQrcode:Cust_id
                   cart_id:cart_id
        ...
        vc.store_id = response[@"data"][@"cart"][@"store_id"];
        vc.cart_id = response[@"data"][@"cart_id"];
        vc.buy_type = @"2";
        vc.money = [response[@"data"][@"cart"][@"total_amount"] floatValue];
        vc.subMoney = [response[@"data"][@"cart"][@"sub_amount"] floatValue];
        vc.receive_type = @"1";
        vc.add_id = @"";
        vc.receive_time = @"";
        vc.memo = @"";
        vc.generalsDic = response[@"data"][@"cart"][@"generals"];
        vc.specialsDic = response[@"data"][@"cart"][@"specials"];
